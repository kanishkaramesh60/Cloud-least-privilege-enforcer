...
for event in logs:
    ...
    # If username is missing or None, use Unknown
...
for user, info in usage.items():
    ...
    report.append({
        "username": user,
        "total_api_calls": info["total_api_calls"],
        "services_used": sorted(
            list(info["services_used"])
        ),
...
for user in report:
    ...
    print(
        f"API Calls : {user['total_api_calls']}"
    )
